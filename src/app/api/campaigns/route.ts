...
export async function GET(request: Request) {
  try {
    ...
    if (!id) {
      const { data, error } = await db.from('campaigns').select('id,title,updated_at').eq('owner_id', owner).order('updated_at', { ascending: false });
      checkDb(error); return Response.json({ campaigns: data });
    }
    z.uuid().parse(id);
    const { data, error } = await db.from('campaigns').select('id,state').eq('id', id).eq('owner_id', owner).single();
    ...
    checkDb(turnError);
    return Response.json({ id, view: playerView(data.state as World), opening: OPENING, turns: turns?.reverse().map(t => ({ ...t.public_result, narration: t.narration })) ?? [] });
...
export async function POST(request: Request) {
  try {
    ...
    checkDb(error);
    return Response.json({ id: data, view: playerView(state), opening: OPENING, turns: [] });
  } catch (e) { return failure(e); }
}
