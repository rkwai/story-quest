...
export const entitySchema = z.object({ id, kind: z.enum(['character', 'npc', 'location', 'faction', 'item']), name: z.string().min(1).max(100), description: text, locationId: id.nullable(), ownerId: id.nullable(), condition: z.string().max(200), knownBy: z.array(id).max(50) });
