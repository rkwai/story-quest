...
export function Adventure({ liveAvailable }: { liveAvailable: boolean }) {
  ...
  async function loadCampaign(id:string) {
    ...
    setCampaign(id); setView(data.view); setTurns(data.turns); setTab('Story');
  ...
  async function showAccount() {
    setAccount(true); setAuthMessage('');
  ...
  async function createCampaign(e:React.FormEvent) {
    ...
    try {
      ...
      setCampaign(data.id); setView(data.view); setTurns([]); setTab('Story'); setInput(''); setAccount(false);
  ...
  async function submit(e:React.FormEvent) {
    e.preventDefault();
    const action = input.trim(); if (!action || submitting.current) return;
    ...
    try {
      ...
      if (code === 'STALE_REVISION' && campaign) await loadCampaign(campaign).catch(()=>undefined);
    } finally { setBusy(false); submitting.current=false; setPhase(''); }
  }
  function resetSample() {
    const fresh=newSample(); setSample(fresh); setCampaign(null); setView(playerView(fresh.world)); setTurns([]); setTab('Story'); setNotice(''); setInput(''); setAccount(false);
  ...
  return <div className="app-shell">
    ...
    <div className="page-heading"><div><p className="eyebrow">{mode==='sample'?'A SAMPLE ADVENTURE':'YOUR LIVING STORY'}</p><h1>{view.title}</h1><p className="subheading">Some stories are waiting to be found. This one is yours to unfold.</p></div><span className="save-indicator"><span/>{mode==='sample'?'Saved on this device':'Your story is saved'}</span></div>
    ...
    <div className="workspace"><main className="main-panel">
      ...
      {tab==='Story' ? <>
        ...
        <div className="composer"><p className="composer-title">What do you do?</p>{turns.length===0 && <div className="suggestions">{suggestions.map(s=><button disabled={busy} key={s} onClick={()=>setInput(s)}>{s}<span>↗</span></button>)}</div>}
    ...
    {account && <div className="modal-scrim" onClick={()=>!busy&&setAccount(false)}><section ref={modal} className="account-modal" role="dialog" aria-modal="true" aria-label="Your adventures" onClick={e=>e.stopPropagation()}><button className="close" aria-label="Close" disabled={busy} onClick={()=>setAccount(false)}><Icon name="close"/></button><p className="eyebrow">STORIES TO RETURN TO</p><h2>Your adventures</h2>
      ...
      <div className="campaign-list">{campaigns.map(c=><button disabled={busy} key={c.id} onClick={async()=>{setBusy(true);try{await loadCampaign(c.id);setAccount(false);}catch{setAuthMessage('Could not open this adventure.');}finally{setBusy(false);}}}>{c.title}<span>Continue ↗</span></button>)}</div>
