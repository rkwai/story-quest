...
import type { InventoryProposal, PublicTurn, World } from '../src/engine/types';
...
type Commit = { p_state: World; p_proposal: InventoryProposal; p_result: PublicTurn };
...
test('using an absent staff blocks every proposed consequence and releases the turn', async t => {
  ...
  const candidate: InventoryProposal = {
  ...
  assert.deepEqual((rejected.details.rejectedProposal as InventoryProposal).itemActions, candidate.itemActions);
...
test('taking a known nearby staff and then using it commits once before narration', async t => {
  ...
  const candidate: InventoryProposal = { ...proposal,
    itemActions: [
