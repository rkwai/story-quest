...
import type { PublicTurn, StoryProposal, World } from '../src/engine/types';
...
type Commit = { p_state: World; p_proposal: StoryProposal; p_result: PublicTurn; p_engine: string };
...
function direction(): StoryProposal['storyPlan']['questUpdates'][number] {
  return {
    ...
    entityIds: ['ashford', 'tower'],
    leads: [{ id: 'ask_tower', text: 'Ask about the tower', action: 'I ask the woman what she knows about the tower.',
...
test('guidance and evidence-backed progress are committed atomically and projected without private world data', async t => {
  ...
  update.leads[0].evidenceFactIds = [];
  update.leads[0].evidenceClaimIds = ['tower_testimony'];
  ...
  assert.equal(fixture.commits[0].p_engine, '1.2.0');
  ...
  assert.ok(result.view.story.leads.some((lead: { id: string }) => lead.id === 'ask_tower'));
...
test('a lead referring to undiscovered evidence rejects the entire attempt before review or commitment', async t => {
  const world = legacyWorld(), original = structuredClone(world);
  const update = direction();
  update.leads[0] = { id: 'secret_lead', text: 'Investigate the Hollow Choir', action: 'Find the buried bell.',
    entityIds: ['order'], evidenceFactIds: ['cause'], evidenceClaimIds: [] };
  const candidate: StoryProposal = { ...proposal,
    itemActions: [{ action: 'drop', itemId: 'journal', quantity: 1, recipientId: null }],
    storyPlan: { focusQuestId: 'mystery', questUpdates: [update], progress: [] },
  };
  const fixture = setup(t, candidate, world);
  const response = await turnPost(request('I put down my notebook and look for a lead.'));
  assert.equal(response.status, 409);
  assert.equal(response.headers.get('cache-control'), 'no-store');
  assert.deepEqual(await response.json(), { error: 'STORY_HIDDEN_LEAD_REFERENCE' });
  assertUncommitted(fixture);
  const failure = fixture.traces.find(trace => trace.stage === 'failed')!;
  assert.deepEqual(failure.details.story, { status: 'blocked', code: 'STORY_HIDDEN_LEAD_REFERENCE' });
  assert.deepEqual((failure.details.rejectedProposal as StoryProposal).storyPlan, candidate.storyPlan);
...
test('clarification releases the turn without changing pacing, guidance, story time or revision', async t => {
  ...
  const candidate: StoryProposal = { ...proposal, kind: 'clarification', elapsedMinutes: 0,
...
test('stalled pacing reaches the existing proposal call and metadata changes cannot pretend to make progress', async t => {
  ...
  assert.deepEqual(story.surfacedLeadIds, ['ask_tower']);
});
