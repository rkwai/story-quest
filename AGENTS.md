...
Rick's Story layout order is action input, newest scene, then scrollable older interactions newest first. Do not display chronology headings such as “Latest interaction” or “Past story.” Reverse presentation only; preserve chronological saved history and replay. Keep pending/errors beside the composer. Within an interaction, show narration before the submitted action and change notes. Bring the response to the player’s own new submission into view when ready; do not scroll to the history bottom or pull readers away when they retry older narration. Cmd+Enter on Mac or Ctrl+Enter elsewhere submits the action; Enter alone inserts a new line.

The DM provides direction through the story. Do not generate, store in new live proposals, or display preset player prompts or a Journal “Known leads” list. Keep quest objectives and stakes in the Journal. Fixed inventory controls and the explicitly labeled offline no-AI preview buttons remain available; they are not generated narrative choices.
...
- Live turns use engine 1.3.0 and prompt `dm-1.5.0` with mandatory `itemActions` and `storyPlan`; live `questUpdates` omit `leads`. Validate item ownership, reach, quantity and availability before other consequences. Do not bypass inventory with generic owner/location updates, direct player-owned item creation, facts or narration. See docs/inventory.md before changing this path.
...
- Read docs/story-direction.md before changing quests, context or pacing. Lore/history is truth; goals and future plans are adaptable possibilities. Answer the player's exact input first, then offer relevant opportunities within the narrative. No forced player action, countdown or quest completion to satisfy a quiet-turn metric. `needsDirection` depends on quiet turns or missing goal metadata, never on absent prompt buttons.
- Preserve 1.0.0, 1.1.0 and 1.2.0 replay behavior. Engine 1.3.0 retains quest objectives, hierarchy and fresh progress evidence while removing generated lead commands from the live contract, context and public projection. Historical 1.2.0 lead data remains compatible. Evidence existence/overlap is not proof of semantic relevance; arbitrary generated text still needs semantic evaluation.
