...
Use a trusted runtime with an explicitly authorized one-time invocation. Do not append the smoke command to the deployment build: automatic approval review rejected that approach because future builds could repeat its paid calls and test-account mutations. Keep the standard build unchanged. Never add a public endpoint that runs paid tests or exports credentials. The script checks the current hosted release, not a still-building deployment. It does not prove email delivery, visual browser behavior, long-campaign continuity or model quality beyond the tested turn.
