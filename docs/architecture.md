...
Engine 1.3.0 uses a typed `storyPlan` with optional quest hierarchy, objectives, stakes, entity links and progress evidence. Live `questUpdates` have no `leads` field. Quests may be world, arc, local or immediate scope; parent scopes must be broader. Fresh evidence is required for reported progress and quest resolution. Quiet-turn counts request better direction, never force a world event. Existing JSONB saves need no migration or reset. Versioned replay preserves the 1.0.0, 1.1.0 and 1.2.0 contracts, including historical lead data, and dispatches new turns to 1.3.0. See [story direction](story-direction.md) for the full contract and semantic limitations.

The director selects at most six quests, preserving the focus's ancestor chain and related evidence. Relevant optional branches fit only after mandatory scene truth, inventory, rules and crossable deadlines. Selection is deterministic retrieval, not player intent classification. Current context strips legacy lead prompts. The DM develops selected goals in its existing proposal call; the existing Jev advisory request additionally reviews unsupported progress and topic drift. `needsDirection` becomes true after three quiet turns or when the focused quest lacks goal metadata. It guides opportunities within the story rather than generating a list of player commands.

The current public projection exposes known quest goals and stakes without leads. Narration expresses direction through committed dialogue, discoveries and consequences; it cannot invent an outcome to create momentum. The Story layout is input, newest scene, then scrollable older scenes without visible chronology headings. The Journal retains goals but no “Known leads” list.
