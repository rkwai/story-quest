...
The paid verifier now requires a fixed run UUID and atomically claims it in a private `verification_runs` ledger before account/model actions. Existing claims cannot be deleted or have their ID changed by the application role. The QA account is retained and reused; later tests reset its active campaign. Each run makes at most one hosted proposal request, one Jev request and one narration request. Normal builds do not run it. This explicit authorization resolves the earlier review requirement for one temporary test invocation followed by restoring the normal build.
